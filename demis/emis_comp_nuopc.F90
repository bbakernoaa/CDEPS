#ifdef CESMCOUPLED
module emis_comp_nuopc
#else
module cdeps_demis_comp
#endif

  !----------------------------------------------------------------------------
  ! This is the NUOPC cap for Demis
  !----------------------------------------------------------------------------
  ...
  private :: demis_comp_advertise
  private :: demis_comp_realize
  private :: demis_comp_run
  private :: get_stream_fields
...
#ifdef CESMCOUPLED
  character(*) , parameter     :: modName =  "(emis_comp_nuopc)"
#else
  character(*) , parameter     :: modName =  "(cdeps_demis_comp)"
#endif
...
!===============================================================================
  ...
  subroutine InitializeAdvertise(gcomp, importState, exportState, clock, rc)
    ...
    namelist / demis_nml / datamode, model_meshfile, model_maskfile, &
    ...
    ! Read demis_nml from nlfilename
    if (my_task == main_task) then
       nlfilename = "demis_in"//trim(inst_suffix)
       open (newunit=nu, file=trim(nlfilename), status="old", action="read")
       call shr_nl_find_group_name(nu, 'demis_nml', status=ierr)

       read (nu,nml=demis_nml,iostat=ierr)
       close(nu)
       if (ierr > 0) then
    ...
    if (trim(datamode) == 'copyall') then
       if (my_task == main_task) write(logunit,*) 'demis datamode = ',trim(datamode)
    else
       call shr_sys_abort(' ERROR illegal demis datamode = '//trim(datamode))
    end if

    ! Advertise the export fields
    call demis_comp_advertise(importState, exportState, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  subroutine InitializeRealize(gcomp, importState, exportState, clock, rc)
    ...
    ! Initialize sdat
    call ESMF_TraceRegionEnter('demis_strdata_init')
    ...
    streamfilename = 'demis.streams'//trim(inst_suffix)
#ifndef DISABLE_FoX
    ...
    call demis_comp_realize(importState, exportState, export_all, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    if (restart_read .and. .not. skip_restart_read) then
       call shr_get_rpointer_name(gcomp, 'emis', current_ymd, current_tod, rpfile, 'read', rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    ! Run demis to create export state
    call demis_comp_run(importState, exportState, current_ymd, current_tod, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
  ...
  subroutine ModelAdvance(gcomp, rc)
    ...
    ! run demis
    call demis_comp_run(importState, exportState, next_ymd, next_tod, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    if (write_restart) then
       call ESMF_TraceRegionEnter('demis_restart')
       call shr_get_rpointer_name(gcomp, 'emis', next_ymd, next_tod, rpfile, 'write', rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       call dshr_restart_write(rpfile, case_name, 'demis', inst_suffix, next_ymd, next_tod, &
            logunit, my_task, sdat, rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       call ESMF_TraceRegionExit('demis_restart')
    endif
  ...
  subroutine ModelFinalize(gcomp, rc)
    ...
    ! local variables
    character(*), parameter :: F00   = "('(demis_comp_final) ',8a)"
    character(*), parameter :: F91   = "('(demis_comp_final) ',73('-'))"
    ...
    if (my_task == main_task) then
       write(logunit,F91)
       write(logunit,F00) ' demis : end of main integration loop'
       write(logunit,F91)
    end if
    call ESMF_LogWrite(subname//' done', ESMF_LOGMSG_INFO)
  ...
  !===============================================================================
  subroutine demis_comp_advertise(importState, exportState, rc)
    ...
    character(CS), allocatable :: stream_fields(:)
    character(CS), allocatable :: stream_vars(:)
    ...
    ! Read stream fields from user_nl_demis_streams
    call get_stream_fields(sdat, stream_fields, stream_vars, num_fields, rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

    ! Add fields from streams to export list
    do i = 1, num_fields
       call dshr_fldList_add(fldsExport, trim(stream_vars(i)))
       if (mainproc) then
          write(logunit,*) 'Adding stream field to export: ', trim(stream_vars(i))
       endif
    end do
    ...
    do while (associated(fldlist))
       ...
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
       call ESMF_LogWrite('(demis_comp_advertise): Fr_emis '//trim(fldList%stdname), ESMF_LOGMSG_INFO)
       fldList => fldList%next
    enddo

    ! TODO: Non snow fields that nead to be added if demis is in cplhist mode
  ...
  end subroutine demis_comp_advertise
  ...
  subroutine demis_comp_realize(importState, exportState, export_all, rc)
    ...
    ! local variables
    character(*), parameter    :: subName = "(demis_comp_realize) "
    ! ----------------------------------------------
    ...
    call dshr_fldlist_realize( exportState, fldsExport, flds_scalar_name, flds_scalar_num,  model_mesh, &
         subname//':demisExport', export_all, rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return

  end subroutine demis_comp_realize

  !===============================================================================
  subroutine demis_comp_run(importState, exportState, target_ymd, target_tod, rc)
    ...
    call ESMF_TraceRegionEnter('Demis_RUN')
    ...
    if (first_time) then
       ...
       ! Create stream-> export state mapping for each field
       do i = 1, num_fields
          call dshr_dfield_add(dfields, sdat, state_fld=trim(stream_vars(i)), &
               strm_flds=(/ trim(stream_fields(i)) /), state=exportState, &
               logunit=logunit, mainproc=mainproc, rc=rc)
          if (ChkErr(rc,__LINE__,u_FILE_u)) return

          if (mainproc) then
             write(logunit,*) 'Created mapping for: ', trim(stream_fields(i)), ' -> ', trim(stream_vars(i))
          endif
       end do

       deallocate(stream_fields)
       deallocate(stream_vars)
       first_time = .false.
    end if

    !--------------------
    ! advance demis streams
    !--------------------

    ! time and spatially interpolate to model time and grid
    call ESMF_TraceRegionEnter('demis_strdata_advance')
    call shr_strdata_advance(sdat, target_ymd, target_tod, logunit, 'demis', rc=rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_TraceRegionExit('demis_strdata_advance')

    ! copy all fields from streams to export state as default
    ! This automatically will update the fields in the export state
    call ESMF_TraceRegionEnter('demis_strdata_copy')
    call dshr_dfield_copy(dfields, sdat, rc)
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    call ESMF_TraceRegionExit('demis_strdata_copy')

    ! determine data model behavior based on the mode
    call ESMF_TraceRegionEnter('demis_datamode')
    select case (trim(datamode))
    case('copyall')
       ! do nothing extra
    end select

    call ESMF_TraceRegionExit('demis_datamode')
    call ESMF_TraceRegionExit('Demis_RUN')

  end subroutine demis_comp_run

#ifdef CESMCOUPLED
end module emis_comp_nuopc
#else
end module cdeps_demis_comp
#endif
