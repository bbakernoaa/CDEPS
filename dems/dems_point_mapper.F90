...
contains
  ...
  subroutine dems_point_mapper_map(ps_list, grid, collapsed, field, rc)
    ...
    ! For 3D grid, dimCount should be 3
    call ESMF_GridGet(grid, minIndex=minIndex, maxIndex=maxIndex, rc=rc)

    ! Get coordinates - assuming they exist and are 3D
    ...
    do p = 1, ps_list%nsources
       call find_nearest_cell(ps_list%sources(p), lon_ptr, lat_ptr, alt_ptr, minIndex, maxIndex, &
  ...
  subroutine find_nearest_cell(source, lon, lat, alt, mins, maxs, best_i, best_j, best_k)
  ...
  end subroutine find_nearest_cell
