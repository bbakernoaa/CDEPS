...
contains
  ...
  subroutine dems_point_mapper_to_locstream(ps_list, lstream, rc)
    ...
    real(r8), pointer :: lat_p(:), lon_p(:), alt_p(:), flux_p(:)
    integer(i4), pointer :: i_p(:), j_p(:), k_p(:)
    integer :: localCount
    integer :: p
    ...
    ! Retrieve pointers from LocStream keys using ESMF_LocStreamGetKey.
    call ESMF_LocStreamGetKey(lstream, keyName='latitude', farray=lat_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='longitude', farray=lon_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='altitude', farray=alt_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='flux', farray=flux_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='grid_i', farray=i_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='grid_j', farray=j_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return

    call ESMF_LocStreamGetKey(lstream, keyName='grid_k', farray=k_p, rc=rc)
    if (rc /= ESMF_SUCCESS) return
